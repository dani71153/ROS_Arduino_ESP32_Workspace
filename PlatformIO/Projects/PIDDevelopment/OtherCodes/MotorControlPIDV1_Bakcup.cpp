...
class Motor {
  private:
    ...
    const float pulsosPorRevolucion = 4320.0; // Pulsos del encoder por revolución
  ...
  public:
    Motor(int enable, int in1, int in2, int encoderA, int encoderB, float kp, float ki, float kd, unsigned long muestreo) 
      : pinEnable(enable), pinIN1(in1), pinIN2(in2), pinEncoderA(encoderA), pinEncoderB(encoderB), kp(kp), ki(ki), kd(kd), 
        ...
        tiempoPrevio(0), posicionEncoder(0), velocidadActual(0) {
    }
    ...
    void setReferenciaVelocidadRPS(float rps) {
      referenciaVelocidad = rps * pulsosPorRevolucion; // Convertir RPS a ticks por segundo
    }
    ...
    void actualizar() {
      ...
      if (tiempoActual - tiempoPrevio >= intervaloMuestreo) {
        ...
        float salidaPID = calcularPID(referenciaVelocidad, velocidadActual);
    ...
    void controlarMotor(float valorPID) {
      if (valorPID > 0) {
        ...
        analogWrite(pinEnable, abs(valorPID));
      } else if (valorPID < 0) {
